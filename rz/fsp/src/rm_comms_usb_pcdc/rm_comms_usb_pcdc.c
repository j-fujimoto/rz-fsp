...
/*******************************************************************************************************************//**
 * @addtogroup RZA_RM_COMMS_USB_PCDC
 * @{
 **********************************************************************************************************************/
...
fsp_err_t RM_COMMS_USB_PCDC_Open (rm_comms_ctrl_t * const p_api_ctrl, rm_comms_cfg_t const * const p_cfg)
{
...
#if BSP_CFG_RTOS == 0
 #if RM_COMMS_USB_PCDC_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(NULL != p_extend->p_gtm);
 #endif

    timer_api_t const * p_timer_api = p_extend->p_gtm->p_api;

    err = p_timer_api->open(p_extend->p_gtm->p_ctrl, p_extend->p_gtm->p_cfg);
    FSP_ERROR_RETURN(FSP_SUCCESS == err || FSP_ERR_ALREADY_OPEN == err, err);

    /* Set callback function for Timer driver */
    err =
        p_timer_api->callbackSet(p_extend->p_gtm->p_ctrl,
    ...
    /* Start timer */
    err = p_timer_api->start(p_extend->p_gtm->p_ctrl);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
#endif
...
fsp_err_t RM_COMMS_USB_PCDC_Close (rm_comms_ctrl_t * const p_api_ctrl)
{
    ...
    /* Close Timer */
    timer_instance_t const * p_timer_instance = p_extend->p_gtm;
