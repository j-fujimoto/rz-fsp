 ...
 * @retval FSP_ERR_INVALID_CHANNEL     Requested channel number is not available on MHU_B_NS.
 **********************************************************************************************************************/
fsp_err_t R_MHU_B_NS_Open (mhu_ctrl_t * const p_ctrl, mhu_cfg_t const * const p_cfg)
{
    ...
    for (uint32_t i = 0; mhu_b_send_type_msg[i] != MHU_B_NS_PRV_ARRAY_END; i++)
    {
    ...
    for (uint32_t i = 0; mhu_b_send_type_rsp[i] != MHU_B_NS_PRV_ARRAY_END; i++)
    {
    ...
    else
    {
        ...
        if (p_instance_ctrl->send_type == MHU_SEND_TYPE_RSP)
        {
            p_instance_ctrl->p_shared_memory_tx =
                (uint32_t *) (g_shmem_base + (MHU_B_NS_SHMEM_CH_SIZE * p_cfg->channel) +
                              MHU_B_NS_RSP_TXD_OFFSET);
        ...
        {
            p_instance_ctrl->p_shared_memory_tx =
                (uint32_t *) (g_shmem_base + (MHU_B_NS_SHMEM_CH_SIZE * mhu_b_snd_ch[p_cfg->channel].msg_channel) +
                              MHU_B_NS_MSG_TXD_OFFSET);
            p_instance_ctrl->p_shared_memory_rx =
                (uint32_t *) (g_shmem_base + (MHU_B_NS_SHMEM_CH_SIZE * mhu_b_snd_ch[p_cfg->channel].msg_channel) +
                              MHU_B_NS_RSP_TXD_OFFSET);
                                                 ...
                                                 mhu_cfg_t const * const    p_cfg)
{
    ...
    FSP_ERROR_RETURN(((1ULL << p_cfg->channel) & BSP_FEATURE_MHU_B_NS_VALID_CHANNEL_MASK), FSP_ERR_INVALID_CHANNEL);
...
/*********************************************************************************************************************
 * MHU_B_NS receive interrupt (for OpenAMP)
 **********************************************************************************************************************/
void metal_irq_isr_wrapper (void)
{
...
/*********************************************************************************************************************
 * MHU_B_NS receive interrupt (for bere mhu_b_ns application).
 **********************************************************************************************************************/
void mhu_b_ns_int_isr (void)
{
    /* Save context if RTOS is used */
