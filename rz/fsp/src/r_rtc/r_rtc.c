...
#ifndef BSP_FEATURE_RTC_CLOCK_SOURCE_UNSELECTABLE
...
static void r_rtc_set_24_hour_format_mode(bool set);
 ...
 *
 * Example:
 * @snippet rzv_r_rtc_example.c R_RTC_Open
 *
...
fsp_err_t R_RTC_Open (rtc_ctrl_t * const p_ctrl, rtc_cfg_t const * const p_cfg)
{
    ...
    if (CRYSTAL_OSCILLATOR_MODE == p_extend->clock_bypass_mode)
    {
        R_BSP_BypassModeCfg(BSP_BYPASS_OSCILLATOR_RTC, BSP_BYPASS_MODE_CRYSTAL_OSC, 0);
    }
    else
    {
        /* Set the bypass mode to external clock receive mode */
        R_BSP_BypassModeCfg(BSP_BYPASS_OSCILLATOR_RTC, BSP_BYPASS_MODE_EXTERNAL_CLK, 0);
    }
#endif
 ...
 #if BSP_FEATURE_RTC_CLOCK_SOURCE_UNSELECTABLE

    /* Clock Source should not be set to LOCO when it is not supported */
    FSP_ERROR_RETURN(RTC_CLOCK_SOURCE_LOCO != p_cfg->clock_source, FSP_ERR_INVALID_ARGUMENT);
 #endif
...
#if RTC_CFG_OPEN_SET_CLOCK_SOURCE && !defined(BSP_FEATURE_RTC_CLOCK_SOURCE_UNSELECTABLE)
     ...
     * at power on. (see section RTC Control Register 4 (RCR4) of the User's Manual)*/
...
fsp_err_t R_RTC_Close (rtc_ctrl_t * const p_ctrl)
{
    ...
    R_BSP_BypassModeCfg(BSP_BYPASS_OSCILLATOR_RTC, BSP_BYPASS_MODE_POWER_DOWN, 0);
#endif
...
/*******************************************************************************************************************//**
 * Implements @ref rtc_api_t::clockSourceSet. Unsupported.
 *
 * @retval FSP_SUCCESS              Initialization was successful and RTC has started.
 * @retval FSP_ERR_ASSERTION        Invalid p_ctrl or p_cfg pointer.
...
fsp_err_t R_RTC_CalendarTimeSet (rtc_ctrl_t * const p_ctrl, rtc_time_t * const p_time)
{
    ...
    /* See section "Setting the Time" of the User's Manual for steps to set the time and error
     * adjustment value */
    ...
    r_rtc_set_24_hour_format_mode(true);
...
fsp_err_t R_RTC_CalendarTimeGet (rtc_ctrl_t * const p_ctrl, rtc_time_t * const p_time)
{
    ...
    uint32_t carry_irq_status = R_RTC->RCR1 & R_RTC_RCR1_CIE_Msk;
     ...
     * therefore they must be read again. See section of "Reading 64-Hz Counter and Time" of the User's Manual*/
    do
    {
 ...
 *
 * Example:
 * @snippet rzv_r_rtc_example.c R_RTC_PeriodicIrqRateSet
 *
...
fsp_err_t R_RTC_PeriodicIrqRateSet (rtc_ctrl_t * const p_ctrl, rtc_periodic_irq_select_t const rate)
{
 ...
 #if BSP_FEATURE_RTC_PES_256_HZ_UNSELECTABLE
    FSP_ASSERT(RTC_PERIODIC_IRQ_SELECT_1_DIV_BY_256_SECOND != rate)
 #endif
#endif
    fsp_err_t err = FSP_SUCCESS;
...
fsp_err_t R_RTC_TimeCaptureGet (rtc_ctrl_t * const p_ctrl, rtc_time_capture_t * const p_time_capture)
{
     ...
     * bits.(see "RSECCPn : Second Capture Register n (n = 0 to 2) (in Calendar Count Mode)" in the
...
static void r_rtc_start_bit_update (uint8_t value)
{
     ...
     * before proceeding (see section "RTC Control Register 2 (RCR2)" of the User's Manual)*/
...
static void r_rtc_software_reset (void)
{
     ...
     * (see section "RTC Control Register 2 (RCR2)" of the User's Manual)*/
    FSP_HARDWARE_REGISTER_WAIT(R_RTC->RCR2_b.RESET, 0U);
}

/*******************************************************************************************************************//**
 * Set the RTC clock hour format to 24 hour format
 *
 * @param[in] set                    Set or Unset 24-hour format
 **********************************************************************************************************************/
static void r_rtc_set_24_hour_format_mode (bool set)
{
    const uint8_t HR24_BIT_MASK      = 0x1;
    uint8_t       register_set_value = 0;
    if (set)
    {
        register_set_value = 1;
    }
    else
    {
        register_set_value = 0;
    }

    R_RTC->RCR2_b.HR24 = (uint8_t) (register_set_value & HR24_BIT_MASK);

    /*
     * See "Notes on Writing to and Reading from Registers" of the User's Manual.
     * The value written is reflected when fourth read operations are performed after writing.
     */
    FSP_HARDWARE_REGISTER_WAIT(R_RTC->RCR2_b.HR24, register_set_value);
}
...
static void r_rtc_set_clock_source (rtc_instance_ctrl_t * const p_ctrl, rtc_cfg_t const * const p_cfg)
{
    ...
    {
        /* Write 0 before writing to the RFRL register after a cold start. (see section "Frequency Register (RFRH/RFRL)" of the User's Manual) */
        R_RTC->RFRH = 0;
     ...
     * reset is completed (see section "RTC Control Register 2 (RCR2)" of the User's Manual) */
...
static fsp_err_t r_rtc_rfrl_validate (uint32_t value)
{
    ...
    /* A value from 0007h through 01FFh can be specified as the frequency comparison value (see section
     * "Frequency Register (RFRH/RFRL)" of the User's Manual) */
...
static void r_rtc_error_adjustment_set (rtc_error_adjustment_cfg_t const * const err_adj_cfg)
{
    ...
    if (mode != R_RTC->RCR2_b.AADJE)
    {
         ...
         * (see section "Procedure for changing the mode of adjustment" of the User's Manual) */
        R_RTC->RADJ = 0U;

        /* When RADJ is modified, check that all the bits are updated before continuing with more processing.
         * (see section "Time Error Adjustment Register (RADJ)" of the User's Manual) */
