...
/* constant used for the dummy write as UM required */
...
fsp_err_t R_SCI_B_I2C_Open (i2c_master_ctrl_t * const p_api_ctrl, i2c_master_cfg_t const * const p_cfg)
{
     ...
     * no benefit to use DTC/DMAC on p_transfer_rx only.
     *
                                       ...
                                       sci_b_i2c_transfer_dir_t  direction)
{
    ...
    else
    {
        ...
        /* Addr total = 3 for Read and 2 for Write (See section "Communication Data Format" in the User's Manual). */
...
static void sci_b_i2c_open_hw_master (sci_b_i2c_instance_ctrl_t * const p_ctrl, i2c_master_cfg_t const * const p_cfg)
{
     ...
     */
    uint32_t ccr1 = (uint32_t) pextend->clock_settings.snfr_value << R_SCI_B0_CCR1_NFCS_Pos;
    ccr1               |= 1 << R_SCI_B0_CCR1_NFEN_Pos; /* Enable Noise filter */
...
void sci_b_i2c_rxi_isr (void)
{
    ...
    FSP_CONTEXT_SAVE
    /* This interrupt is invoked once DTC/DMAC supported Read transfer is completed. Nothing to be done here. */
...
static void sci_b_i2c_txi_handler (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (0U == (p_ctrl->p_reg->ISR & R_SCI_B0_ISR_IICACKR_Msk))
    {
        ...
        /* This is the first interrupt after the completion of DTC/DMAC operation, ignore it */
 ...
 * @param[in]     p_transfer                 Pointer to transfer instance structure
                                               ...
                                               sci_b_i2c_transfer_interrupt_trigger_t trigger)
{
    ...
    {
 #ifdef SCI_B_I2C_CFG_DMAC_ENABLE
 ...
 #else
        p_info->transfer_settings_word = SCI_B_I2C_PRV_DTC_RX_FOR_READ_TRANSFER_SETTINGS;
 #endif
        p_info->p_src = (void *) (&(p_ctrl->p_reg->RDR));
    }
    else
    {
        /* In case of read operation using DTC/DMAC, the TXI interrupt will trigger the DTC/DMAC to write 0xFF into TDR */
