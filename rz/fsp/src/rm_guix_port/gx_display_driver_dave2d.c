...
/* A flag in gx_glyph_map_size to indicate run-length encoding is used. */
#define GX_GLYPH_RLE_FLAG    (0x8000)

/* The number of fractional bits for the fixed-point format required by D/AVE2D. */
#define DAVE2D_FIXED_POINT_SHIFT    (6)

/* Macro to select the supported alpha palette for glyph rendering. */
#define DAVE2D_ENABLE_ALPHA_PALETTE_MODE    (1)
/***********************************************************************************************************************
 * Typedef definitions
 **********************************************************************************************************************/
#if (GX_RENESAS_DAVE2D_DRAW == 1)
                                            ...
                                            const GX_GLYPH  * glyph)
{
    ...
    if (compressed_glyph->gx_glyph_map_size & GX_GLYPH_RLE_FLAG)
    {
        mode |= d2_mode_rle;
    }
...
VOID _gx_dave2d_pie_fill (GX_DRAW_CONTEXT * context, INT xcenter, INT ycenter, UINT r, INT start_angle, INT end_angle)
{
    ...
    GX_FIXED_VAL s_angle = (GX_FIXED_VAL) -start_angle;
    GX_FIXED_VAL e_angle = (GX_FIXED_VAL) -end_angle;

    sin1 = (INT) _gx_utility_math_sin((GX_FIXED_VAL) ((s_angle - 90) << GX_FIXED_VAL_SHIFT));
    cos1 = (INT) _gx_utility_math_cos((GX_FIXED_VAL) ((s_angle - 90) << GX_FIXED_VAL_SHIFT));

    sin2 = (INT) _gx_utility_math_sin((GX_FIXED_VAL) ((e_angle + 90) << GX_FIXED_VAL_SHIFT));
    cos2 = (INT) _gx_utility_math_cos((GX_FIXED_VAL) ((e_angle + 90) << GX_FIXED_VAL_SHIFT));
    ...
    CHECK_DAVE_STATUS(d2_renderwedge(dave, (d2_point) (D2_FIX4((USHORT) xcenter)),
                                     ...
                                     (d2_s32) ((UINT) cos1 << DAVE2D_FIXED_POINT_SHIFT),
                                     (d2_s32) ((UINT) sin1 << DAVE2D_FIXED_POINT_SHIFT), (d2_s32) ((UINT) cos2 << DAVE2D_FIXED_POINT_SHIFT),
                                     (d2_s32) ((UINT) sin2 << DAVE2D_FIXED_POINT_SHIFT), flags))
...
#if (DAVE2D_ENABLE_ALPHA_PALETTE_MODE == 1)
    gx_dave2d_color0_set(dave, text_color);
    gx_dave2d_blend_mode_set(dave, d2_bm_alpha, d2_bm_one_minus_alpha);
...
{
    GX_PARAMETER_NOT_USED(canvas);
    GX_PARAMETER_NOT_USED(composite);
}
