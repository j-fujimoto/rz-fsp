...
/* According to the UM, the bitrate B is calculated using the formula:
 *
 * B = TCLK / ( (N+1) * 8 * 2^(2n-1) )
 *
 * To calculate the minimal bitrate, we need to find the maximum possible DIV value.
 * This occurs when both N and n are set to their maximum values, 0xFF and 3, respectively.
 */
...
#if (SCI_B_SPI_DTC_SUPPORT_ENABLE == 1) || (SCI_B_SPI_DMAC_SUPPORT_ENABLE == 1)
...
void sci_b_spi_tx_dmac_callback(sci_b_spi_instance_ctrl_t * p_ctrl);
void sci_b_spi_rx_dmac_callback(sci_b_spi_instance_ctrl_t * p_ctrl);
 ...
 *
 * @return     See @ref RZV_RENESAS_ERROR_CODES or functions called by this function for other possible return codes. This
 *             function calls:
 ...
 * @retval     FSP_ERR_INVALID_ARGUMENT  Clock source is invalid.
                                        ...
                                        sci_b_spi_div_setting_t * sclk_div)
{
    ...
    FSP_ASSERT(bitrate);
    FSP_ASSERT(bitrate >= (peripheral_clock / SCI_B_SPI_PRV_CLK_MAX_DIV));
#endif
...
static void r_sci_b_spi_hw_config (sci_b_spi_instance_ctrl_t * const p_ctrl)
{
    ...
    {
        /* set CCR3.CPOL = 0 for clock polarity as low during idle. */
    ...
    /* Write all settings except MOD[2:0] to CCR3 (See "SCI Initialization Flowchart Example (Clock Synchronous Mode)" in the User's Manual). */
...
static fsp_err_t r_sci_b_spi_transfer_config (sci_b_spi_instance_ctrl_t * const p_ctrl)
{
 ...
 #if SCI_B_SPI_DMAC_SUPPORT_ENABLE == 1
 ...
 * @return     See @ref RZV_RENESAS_ERROR_CODES or functions called by this function for other possible return codes. This
 *             function calls:
 *               - @ref RZV::transfer_api_t::reconfigure
 **********************************************************************************************************************/
static fsp_err_t r_sci_b_spi_write_read_common (sci_b_spi_instance_ctrl_t * const p_ctrl,
                                                void const                      * p_src,
                                                void                            * p_dest,
                                                uint32_t const                    length)
{
    ...
    if ((NULL != p_ctrl->p_cfg->p_transfer_rx) && (NULL != p_dest))
    {
        ...
        fsp_err_t err = p_transfer->p_api->reconfigure(p_transfer->p_ctrl, p_info);
 #else
 ...
 * been successfully transferred to the hardware register. And it will enable the transmit end IRQ (TEND) and disable
 * the transmit empty IRQ (TXI).
 **********************************************************************************************************************/
void sci_b_spi_tx_dmac_callback (sci_b_spi_instance_ctrl_t * p_ctrl)
{
    uint32_t ccr0_temp = p_ctrl->p_reg->CCR0;
