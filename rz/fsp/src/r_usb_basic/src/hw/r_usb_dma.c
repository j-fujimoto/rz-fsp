...
static usb_utr_t g_usb_cstd_int_dma[USB_INT_BUFSIZE];
   #endif /* !defined(BSP_MCU_GROUP_RZT2M) && !defined(BSP_MCU_GROUP_RZT2L) && !defined(BSP_MCU_GROUP_RZA3UL) */
...
{
    {
        ...
        {(uint32_t) &USB_M0->CFIFO,             (uint32_t) &USB_M0->CFIFO_hword.H,
         (uint32_t) &USB_M0->CFIFO_byte.HH}, /* USBA CFIFO  adr Little */
    ...
    #else /* !defined(BSP_MCU_GROUP_RZA3UL) */
        {(uint32_t) (uintptr_t) &USB_M0->CFIFO, (uint32_t) (uintptr_t) &USB_M0->CFIFO_hword.H,
         (uint32_t) (uintptr_t) &USB_M0->CFIFO_byte.HH},                                           /* USBA CFIFO  adr Little */
        {0,                                     0,                                            0 }, /* USBA D0FIFO adr Little */
        {0,                                     0,                                            0 } /* USBA D1FIFO adr Little */
        ...
        {0,                                     0,                                            0 }, /* USBA D0FIFO adr Big */
        {0,                                     0,                                            0 } /* USBA D1FIFO adr Big */
   #endif /* USB_CFG_ENDIAN == USB_CFG_BIG */
  #endif /* #if !defined(BSP_MCU_GROUP_RZT2M) && !defined(BSP_MCU_GROUP_RZT2L) && !defined(BSP_MCU_GROUP_RZA_USB) */
    },
  #if USB_NUM_USBIP == 2
    {
        /* IP1 */
    ...
    #else /* !defined(BSP_MCU_GROUP_RZA3UL) */
        {0,                                     0,
         0},                           /* USBA CFIFO  adr Little */
        {0,                                     0,
         0},                           /* USBA D0FIFO adr Little */
        {0,                                     0,
         0},                           /* USBA D1FIFO adr Little */
    #endif /* !defined(BSP_MCU_GROUP_RZA3UL) */
...
void usb_cstd_dma_send_continue (usb_utr_t * ptr, uint16_t useport)
{
    ...
    if (p_data_cnt)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                p_src_adr =
                    (uint8_t *) (uintptr_t) ((uint32_t) (uintptr_t) p_data_ptr + g_usb_cstd_dma_size[ip][channel]);
  ...
  #if (2 == USB_NUM_USBIP)
   #if defined(BSP_MCU_GROUP_RA6M3)
                    g_usb_cstd_dma_fraction_size[ip][channel] = g_usb_cstd_dma_size[1][channel] & USB_BIT_MBW32; /* fraction size(1-3) */
                    dma_size = (uint16_t) (dma_size & ~USB_BIT_MBW32);
   #else                                                                                                         /* defined(BSP_MCU_GROUP_RA6M3) */
                ...
                if (0 != dma_size)
                {
                    ...
                    /* DMA0 1byte trans */
                    usb_cstd_dma_send_restart(ptr, (uint32_t) (uintptr_t) p_src_adr, dma_size, pipe);
                }
                else
                {
                    cpu_write = true;  /* Set flag for CPU FIFO Write */
