...
#define XSPI_QSPI_PRV_BMCTL_DEFAULT_VALUE             (0x03U)
 ...
 *
 * Example:
 * @snippet rzv_r_xspi_qspi_example.c R_XSPI_QSPI_Open
 *
...
fsp_err_t R_XSPI_QSPI_Open (spi_flash_ctrl_t * p_ctrl, spi_flash_cfg_t const * const p_cfg)
{
    ...
    /* Temporarily opened for using APIs in user callback function */
    p_instance_ctrl->open = XSPI_QSPI_PRV_OPEN;

    /* Call user-defined pre-initialize callback */
    if (p_cfg_extend->pre_init)
    {
        fsp_err_t err;
        err = p_cfg_extend->pre_init(&g_spi_flash_on_xspi_qspi, p_instance_ctrl);
        if (FSP_SUCCESS != err)
        {
    ...
    if (XSPI_QSPI_CHIP_SELECT_0 == p_cfg_extend->chip_select)
    {
        p_instance_ctrl->p_reg->WRAPCFG_b.DSSFTCS0 = 0x0000;
    }
    else
    {
        p_instance_ctrl->p_reg->WRAPCFG_b.DSSFTCS1 = 0x0000;
    }
    ...
    if (XSPI_QSPI_CHIP_SELECT_0 == p_cfg_extend->chip_select)
    {
        p_instance_ctrl->p_reg->BMCTL0_b.CS0ACC = XSPI_QSPI_PRV_BMCTL_DEFAULT_VALUE;
    }
    else
    {
        p_instance_ctrl->p_reg->BMCTL0_b.CS1ACC = XSPI_QSPI_PRV_BMCTL_DEFAULT_VALUE;
    }
    ...
    /* Call user-defined post-initialize callback */
    if (p_cfg_extend->post_init)
    {
        fsp_err_t err;
        err = p_cfg_extend->post_init(&g_spi_flash_on_xspi_qspi, p_instance_ctrl);
        if (FSP_SUCCESS != err)
        {
            p_instance_ctrl->open = 0;
 ...
 *
 * Example:
 * @snippet rzv_r_xspi_qspi_example.c R_XSPI_QSPI_DirectTransfer
 *
...
static fsp_err_t r_xspi_qspi_xip (xspi_qspi_instance_ctrl_t * p_instance_ctrl, uint8_t code, bool enter_mode)
{
    ...
    dummy = *(volatile uint8_t *) (BSP_FEATURE_XSPI_START_ADDRESS +
                                   (chip_select * BSP_FEATURE_XSPI_DEVICE_CS0_1_ADDRESS_DELTA));
    (void) unit;
