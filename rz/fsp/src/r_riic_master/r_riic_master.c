...
#if RIIC_MASTER_CFG_DMAC_B_ENABLE
void             riic_master_rx_dmac_b_callback(iic_master_instance_ctrl_t * p_ctrl);
void             riic_master_tx_dmac_b_callback(iic_master_instance_ctrl_t * p_ctrl);
...
static void iic_master_notify (iic_master_instance_ctrl_t * const p_ctrl, i2c_master_event_t const event)
{
    ...
    /* Stop any DMAC_B assisted transfer for tx */
    ...
    /* Stop any DMAC_B assisted transfer for rx */
...
static fsp_err_t iic_master_run_hw_master (iic_master_instance_ctrl_t * const p_ctrl)
{
    /* The ratio I2CLK (System Clock)/(RIIC source clock/IIC) gives the CPU cycles for 1 ICBRL count.
     * IIC is division ratio of internal reference clock of RIICn.
     * The internal reference clock (IIC_CLK) is calculated as (RIIC source clock/IIC).
     ...
     */
    riic_master_extended_cfg_t * p_extend = (riic_master_extended_cfg_t *) p_ctrl->p_cfg->p_extend;
    uint32_t i2_clk        = R_FSP_SystemClockHzGet(BSP_FEATURE_BSP_SYSTEM_CORE_CLOCK);
    uint32_t p0_clk        = R_FSP_SystemClockHzGet(BSP_FEATURE_IIC_SOURCE_CLOCK);
...
static void iic_master_rxi_master (iic_master_instance_ctrl_t * p_ctrl)
{
...
#if RIIC_MASTER_CFG_DMAC_B_ENABLE
        uint8_t volatile const * p_iic_master_rx_buffer = (uint8_t *) (&(p_ctrl->p_reg->ICDRR));
    ...
    /* If this is the interrupt that got fired after DMAC_B transfer,
     * ignore it as the DMAC_B has already taken care of the data transfer */
    ...
    else
    {
        ...
        p_ctrl->p_reg->ICMR2 = ((uint8_t) 0x02UL |
                                (uint8_t) (IIC_MASTER_TIMEOUT_MODE_SHORT ==
                                           ((riic_master_extended_cfg_t *) p_ctrl->p_cfg->
...
#if RIIC_MASTER_CFG_DMAC_B_ENABLE
        uint8_t volatile const * p_iic_master_tx_buffer = (uint8_t *) (&(p_ctrl->p_reg->ICDRT));
