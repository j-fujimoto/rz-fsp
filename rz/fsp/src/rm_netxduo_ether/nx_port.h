...
/* Define several macros for the error checking shell in NetX.  */
 ...
 #define NX_INIT_AND_THREADS_CALLER_CHECKING    if (((_tx_thread_system_state) &&                         \
                                                     (_tx_thread_system_state < ((ULONG) 0xF0F0F0F0))) || \
                                                    (_tx_thread_current_ptr == &_tx_timer_thread)) {      \
        return (NX_CALLER_ERROR);}

 #define NX_NOT_ISR_CALLER_CHECKING             if ((_tx_thread_system_state) &&                        \
                                                    (_tx_thread_system_state < ((ULONG) 0xF0F0F0F0))) { \
        return (NX_CALLER_ERROR);}
...
CHAR _nx_version_id[] =
    "Copyright (c) Microsoft Corporation. All rights reserved.  *  NetX Duo Cortex-A5x/AC6 Version 6.1.11 *";
#else
