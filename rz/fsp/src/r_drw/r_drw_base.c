...
void d1_setregister (d1_device * handle, d1_int_t deviceid, d1_int_t index, d1_long_t value)
{
    ...
    switch (deviceid)
    {
 ...
 #endif
                    handle_flex->pp_dlist_indirect_start = (uint32_t *) ((uint32_t **) (p_dlist) + 1);
                }
                else
                {
                    /* Write data to DLISTSTART register. */
 #if (BSP_FEATURE_BSP_HAS_MMU_SUPPORT)
                    va = (uint64_t) (uint32_t) value;
                    R_MMU_VAtoPA(&g_mmu_ctrl, va, (void *) &pa);
                    ((uint32_t *) R_DRW)[index] = (uint32_t) pa;
 #else
        ...
        {
#if DRW_CFG_USE_DLIST_INDIRECT
            handle_flex->dlist_indirect_enable = (int32_t) value;
#else
