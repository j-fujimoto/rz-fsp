...
fsp_err_t R_SPI_B_Open (spi_ctrl_t * p_api_ctrl, spi_cfg_t const * const p_cfg)
{
    ...
    /* When the TXI Interrupt is handled in the RXI ISR, a TX Transfer instance must be present if there is a
     * RX Transfer instance present otherwise the TXI Interrupts will not be processed. */
...
fsp_err_t R_SPI_B_Close (spi_ctrl_t * const p_api_ctrl)
{
    ...
    /* Dummy read to ensure that status flag is cleared. */
    FSP_REGISTER_READ(p_ctrl->p_regs->SPSR);
...
fsp_err_t R_SPI_B_CalculateBitrate (uint32_t bitrate, spi_b_clock_source_t clock_source, rspck_div_setting_t * spck_div)
{
    ...
    {
        desired_divider = R_FSP_SystemClockHzGet(BSP_FEATURE_SPI_PCLK);
    }
    else
    {
...
#else
        desired_divider = R_FSP_SystemClockHzGet(BSP_FEATURE_SPI_TCLK(clock_source));
#endif
    }
 ...
 *                             calls @ref RZV::transfer_api_t::open.
 **********************************************************************************************************************/
static fsp_err_t r_spi_b_transfer_config (spi_cfg_t const * const p_cfg)
{
    fsp_err_t err = FSP_SUCCESS;
...
static void r_spi_b_hw_config (spi_b_instance_ctrl_t * p_ctrl)
{
...
#endif
    {
        /* Transmit Buffer Empty IRQ must be enabled for Transfer even if TRANSMIT_FROM_RXI is enabled. */
    ...
    /* Set clock source */
    if (p_extend->clock_source == SPI_B_CLOCK_SOURCE_PCLK)
    {
        spcr |= R_SPI_B0_SPCR_BPEN_Msk;
    }
    ...
    if (SPI_B_SSL_MODE_SPI == p_extend->spi_clksyn)
    {
        ...
        spcmd0 |=
            (uint32_t) (((!p_ctrl->p_cfg->operating_mode) & p_extend->ssl_level_keep) << R_SPI_B0_SPCMD0_SSLKP_Pos);
    }
    else
    {
    ...
    p_ctrl->p_regs->SPDCR2 = spdcr2;

    /* Set all SPCR bits except MSTR */
    p_ctrl->p_regs->SPCR = spcr & ~R_SPI_B0_SPCR_MSTR_Msk;

    /* Read back SPCR to ensure 1 TCLK has passed
     * (see Figure 'Example of Initialization Flowchart in Master Mode (SPI Operation)' Note 1 in the User's Manual) */
    p_ctrl->p_regs->SPCR;
 ...
 * @param[in]  p_ctrl          pointer to control structure.
 *
 * Note: When not using the Transfer to transmit, this function pre-loads the SPI shift-register and shift-register-buffer
                                            ...
                                            spi_bit_width_t const bit_width)
{
 ...
 #if SPI_B_DMAC_SUPPORT_ENABLE
        p_info->src_size       = size;
        p_info->dest_size      = size;
        p_info->length         = length * byte_width;
