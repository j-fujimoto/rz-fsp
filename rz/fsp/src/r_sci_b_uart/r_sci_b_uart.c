...
#define SCI_B_UART_TDR_TDAT_MASK_9BITS         (0x000001FFU)
#define SCI_B_UART_TDR_TDAT_MASK               (0xFFFFFE00U)
...
#if BSP_FEATURE_SCI_UART_FIFO_DEPTH == 32
 #define SCI_B_UART_FCR_TRIGGER_MASK           (0x1F)
#else
...
void sci_b_uart_tx_dmac_callback(sci_b_uart_instance_ctrl_t * p_ctrl);
void sci_b_uart_rx_dmac_callback(sci_b_uart_instance_ctrl_t * p_ctrl);
...
fsp_err_t R_SCI_B_UART_Open (uart_ctrl_t * const p_api_ctrl, uart_cfg_t const * const p_cfg)
{
 ...
 #ifdef BSP_FEATURE_SCI_UART_SUPPORT_HALF_DATA_FUNCTION
    if (((sci_b_uart_extended_cfg_t *) p_cfg->p_extend)->half_data_setting.enable == SCI_B_UART_HALF_DATA_ENABLE)
    {
        FSP_ERROR_RETURN(((sci_b_uart_extended_cfg_t *) p_cfg->p_extend)->p_baud_setting->baudrate_bits_b.brme == 0 &&
                         ((sci_b_uart_extended_cfg_t *) p_cfg->p_extend)->p_baud_setting->baudrate_bits_b.abcs == 0 &&
                         ((sci_b_uart_extended_cfg_t *) p_cfg->p_extend)->p_baud_setting->baudrate_bits_b.abcse == 0,
                         FSP_ERR_INVALID_ARGUMENT);
    }
 #endif
    ...
    /* Initialize registers as defined in section "SCI Initialization (Asynchronous Mode)" in the manual */
...
fsp_err_t R_SCI_B_UART_Read (uart_ctrl_t * const p_api_ctrl, uint8_t * const p_dest, uint32_t const bytes)
{
    ...
    if (NULL != p_ctrl->p_cfg->p_transfer_rx)
    {
        ...
        p_info->p_dest = (void *) p_dest;
        p_info->length = bytes;

        err = p_transfer_rx->p_api->reconfigure(p_transfer_rx->p_ctrl, p_info);
  #else
...
fsp_err_t R_SCI_B_UART_Write (uart_ctrl_t * const p_api_ctrl, uint8_t const * const p_src, uint32_t const bytes)
{
     ...
     * Please refer "Serial Data Transmission (Asynchronous Mode)" section in the manual */
    p_ctrl->p_reg->CCR0 &= (uint32_t) ~(R_SCI_B0_CCR0_TE_Msk);
    ...
    if ((NULL != p_ctrl->p_cfg->p_transfer_tx) && p_ctrl->tx_src_bytes)
    {
        ...
        p_info->p_src  = (void const *) p_ctrl->p_tx_src;
        p_info->length = bytes;

        err = p_transfer_tx->p_api->reconfigure(p_transfer_tx->p_ctrl, p_info);
  #else
...
fsp_err_t R_SCI_B_UART_Abort (uart_ctrl_t * const p_api_ctrl, uart_dir_t communication_to_abort)
{
    ...
    if (UART_DIR_TX & communication_to_abort)
    {
         ...
         * rewriting the control register. Please refer "Communication Enable Status Register(CESR)"
                                                  ...
                                                  uint32_t                           sci_buffer_address)
{
 ...
 #ifdef SCI_B_UART_CFG_DMAC_SUPPORTED
...
static void r_sci_b_uart_config_set (sci_b_uart_instance_ctrl_t * const p_ctrl, uart_cfg_t const * const p_cfg)
{
    ...
    /* Configure the Half Data Function */
    uint8_t hcr = 0;

    if (SCI_B_UART_HALF_DATA_ENABLE == p_extend->half_data_setting.enable)
    {
        /* Set the HDEN bit in the HCR register to enable the half data function */
        hcr |= 1U << R_SCI_B0_HCR_HDEN_Pos;
    }

    /* If the half data function is enabled, set the enable bit.
     * If the function is disabled, reset the HCR register to 0. The hardware doesn't do this automatically. */
    p_ctrl->p_reg->HCR = hcr;
#endif
}
...
static void r_sci_b_uart_fifo_cfg (sci_b_uart_instance_ctrl_t * const p_ctrl)
{
    if (0U != p_ctrl->fifo_depth)
    {
        ...
        /* If DTC or DMAC is used keep the receive trigger at the default level of 0. */
        if (NULL == p_ctrl->p_cfg->p_transfer_rx)
  #endif
        {
        ...
        if (NULL != p_ctrl->p_cfg->p_transfer_tx)
        {
            /* When using the DMAC, setting TTRG to (FIFO_DEPTH - 1) is recommended by the UM. For more information, refer to the
             * manual and navigate to the FCR Register's TTRG bits. */
        ...
        if (0U != p_ctrl->fifo_depth)
        {
            ...
            for (uint32_t cnt = fifo_count; (cnt < p_ctrl->fifo_depth) && p_ctrl->tx_src_bytes; cnt++)
            {
                if ((2U == p_ctrl->data_bytes))
                {
                    ...
                    p_ctrl->p_reg->TDR =
                        ((uint32_t) (*(uint16_t *) (p_ctrl->p_tx_src)) & SCI_B_UART_TDR_TDAT_MASK_9BITS) |
                        SCI_B_UART_TDR_TDAT_MASK;
                }
                else
                {
...
/*******************************************************************************************************************//**
 * This function serves as the callback for handling the termination of a DMAC transfer.
 *
 * It is invoked from the DMAC's ISR whenever a transfer finished, signifying that all data within the memory buffer has
 * been successfully transferred to the hardware register for the specified transfer length. And it will enable the
 * transmit end IRQ (TEND), disable the transmit empty IRQ (TXI), and invoke the user's callback function to notify the
 * UART_EVENT_TX_DATA_EMPTY event.
 **********************************************************************************************************************/
void sci_b_uart_tx_dmac_callback (sci_b_uart_instance_ctrl_t * p_ctrl)
{
    uint32_t ccr0_temp = p_ctrl->p_reg->CCR0;
 ...
 * This function serves as the callback for handling the termination of a DMAC transfer.
 *
 * It is invoked from the DMAC's ISR whenever a transfer finished, signifying that all data has been successfully
 * transferred to the memory buffer from the hardware register for the specified transfer length. And it will invoke the
 * user's callback function to notify the UART_EVENT_RX_COMPLETE event.
 **********************************************************************************************************************/
void sci_b_uart_rx_dmac_callback (sci_b_uart_instance_ctrl_t * p_ctrl)
{
    p_ctrl->rx_dest_bytes = 0;
