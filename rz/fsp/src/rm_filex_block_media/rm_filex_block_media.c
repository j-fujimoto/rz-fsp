...
/*******************************************************************************************************************//**
 * @addtogroup RZA_RM_FILEX_BLOCK_MEDIA
 * @{
 **********************************************************************************************************************/
 ...
 * @return See @ref RZA_RENESAS_ERROR_CODES or functions called by this function for other possible return codes.
 *         This function calls:
 *             * @ref RZA::rm_block_media_api_t::open
 *             * @ref RZA::rm_block_media_api_t::mediaInit
 *             * @ref RZA::rm_block_media_api_t::infoGet
 *             * @ref RZA::rm_block_media_api_t::close
 *********************************************************************************************************************/
static fsp_err_t rm_filex_block_media_driver_request_handler (FX_MEDIA * p_fx_media)
{
    fsp_err_t err;
...
void rm_filex_block_media_memory_callback (rm_block_media_callback_args_t * p_args)
{
    ...
    if ((RM_BLOCK_MEDIA_EVENT_POLL_STATUS) &p_args->event)
    {
        if (NULL != p_instance_ctrl->p_cfg->p_callback)
        {
            uint32_t       DAIF_register = 0;
            const uint32_t I_F_bits      = ((1 << 6) | (1 << 7));
            __asm volatile ("MRS %0, DAIF" : "=r" (DAIF_register));

            /* Check if POLL_STATUS is from an ISR or a THREAD.
             * If from an ISR, handling is done later with WAIT_END event. */
            if (0 == (DAIF_register & I_F_bits))
            {
 ...
 * @return See @ref RZA_RENESAS_ERROR_CODES or functions called by this function for other possible return codes.
 *         This function calls:
 *             * @ref RZA::rm_block_media_api_t::write
 **********************************************************************************************************************/
static fsp_err_t rm_filex_block_media_write (rm_filex_block_media_instance_ctrl_t * p_instance_ctrl,
                                             uint8_t                              * p_src_address,
                                             ULONG                                  block_address,
                                             ULONG                                  num_blocks)
{
    ...
    err = p_block_media->p_api->write(p_block_media->p_ctrl,
                                      p_src_address,
 ...
 * @return See @ref RZA_RENESAS_ERROR_CODES or functions called by this function for other possible return codes.
 *         This function calls:
 *             * @ref RZA::rm_block_media_api_t::read
 **********************************************************************************************************************/
static fsp_err_t rm_filex_block_media_read (rm_filex_block_media_instance_ctrl_t * p_instance_ctrl,
                                            uint8_t                              * p_dest_address,
                                            ULONG                                  block_address,
                                            ULONG                                  num_blocks)
{
    ...
    err = p_block_media->p_api->read(p_block_media->p_ctrl,
                                     p_dest_address,
                                     (uint32_t) block_address,
                                     (uint32_t) num_blocks);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
